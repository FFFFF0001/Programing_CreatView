...
- (void)programing
{
    id haha = [Person creatPerson:^(Person<nameProtocol> *person) {
        person.name.set(@"a").age.set(@"20");
    ...
    UIView *view = [CreatControls creatControls:^(CreatControls<CreatControlsProtocol> *controls) {
        controls.label.backColor(backColor).rect(frame1).title(@"测试").middle_alignment.titleColor(textColor).font(font1);
    }];
    ...
    UIView *view1 = [CreatControls creatControls:^(CreatControls<CreatControlsProtocol> *controls) {
        controls.label.backColor(backColor).rect(frame2).title(@"测试2").titleColor(textColor).font(font1).cornerRadius(5.0f).borderWidth(1.0f).borderColor([UIColor blackColor].CGColor);
    }];
    [self.view addSubview:view1];

    
    CGRect frame3 = CGRectMake(100, 300, 100, 30);
    UITextView *tf = [CreatControls creatControls:^(CreatControls<CreatControlsProtocol> *controls) {
